...
try {
  ...
  const existing = await prisma.importFile.findFirst({ where: { OR: [{ id: source.id }, { hash: source.hash }] } });
  if (existing) {
    if (existing.hash !== source.hash) throw new Error('Registered source has a different checksum');
    await prisma.$transaction(async tx => {
      const key = existing.objectKey;
      for (const [fileKey, body] of [[key, bytes], [key + '.json', JSON.stringify({ ...source, id: existing.id })], [key + '-raw.json', raw]] as const) {
        if (!await tx.storedFile.findUnique({ where: { key: fileKey }, select: { key: true } })) await putFile(fileKey, body, tx);
      }
    }, { timeout: 20000 });
